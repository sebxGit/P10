...
def objective(args, trial):
    ...
    if isinstance(model, torch.nn.Module):
      ...
      ## deepspeed
      deepspeed_config = DeepSpeedStrategy(stage=2, process_group_backend="gloo")

      trainer = L.Trainer(max_epochs=params['max_epochs'], log_every_n_steps=0,
                          enable_checkpointing=False, strategy=deepspeed_config)
