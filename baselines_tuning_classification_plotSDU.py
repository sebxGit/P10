...
def add_features(hourly_df, dataset_name, historical_feature, weather_df=None):
  ...
  # 12h
  hourly_df['Energy_Consumption_12h'] = hourly_df[historical_feature].shift(12)

  # 24h
  hourly_df['Energy_Consumption_24h'] = hourly_df[historical_feature].shift(24)

  # 1 week
  hourly_df['Energy_Consumption_1w'] = hourly_df[historical_feature].shift(24*7)
...
def objective(args, trial, study):
    params = {
        ...
        'learning_rate': 0.004895508586161393,      # Learning rate for the optimizer
        'max_epochs': 1900,                         # Maximum number of epochs
        'patch_len': 14,                            # Patch length for xPatch
        'padding_patch': 'None',                    # Padding type for patches
        'revin': 0,                                 # Reversible normalization flag
        'ma_type': 'reg',                           # Moving average type
        'alpha': 0.9217499028959255,                # Alpha parameter for xPatch
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'Predictions/{args.dataset}_{args.model}_{huber_loss_scores}_plot.png')
    plt.show()
    plt.clf()
    plt.close()
    print(
        f"model: {args.model}, loss: {total_huber_loss_score}, {huber_loss_scores}, recall: {total_recall_score}, trial: {trial.number}")
    exit()
    ...
    # plt.legend()
    # plt.tight_layout()
    # plt.savefig(f'Predictions/{args.dataset}_{args.model}_{total_recall_score}_{total_huber_loss_score}_plot.png')
    # plt.show()
    # plt.clf()
    # plt.close()
    # print(
    #     f"model: {args.model}, loss: {total_recall_score_float}, {total_huber_loss_score}")
    # exit()
