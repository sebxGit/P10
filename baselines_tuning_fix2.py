...
class LightningModel(L.LightningModule):
  ...
  def predict_step(self, batch, batch_idx):
    ...
    gathered = self.all_gather(y_hat) # gather all predictions from all GPUs

    return gathered # gather all predictions from all GPUs
...
def get_actuals_and_prediction_flattened(colmod, predictions):
    # 1) Gather and flatten all predictions:
    preds = []
    for tensor in predictions:
        # tensor may have shape [world_size, batch_size, ...] or just [batch_size,...]
        arr = tensor.detach().cpu().numpy().reshape(-1)
        preds.extend(arr.tolist())

    # 2) Load every target from the predict_dataloader and flatten:
    actuals = []
    for x, y in colmod.predict_dataloader():
        actuals.extend(y.detach().cpu().numpy().reshape(-1).tolist())
