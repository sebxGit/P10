...
def get_baseloads_and_parts(colmod, y_pred, actuals):
  if args.dataset == "Colorado":
    ...
    baseload1 = pd.read_csv('Colorado/ElectricityDemandColorado/ColoradoDemand_Part2.csv')
    baseload1['Timestamp (Hour Ending)'] = pd.to_datetime(baseload1['Timestamp (Hour Ending)'])

    range1_start = pd.Timestamp('2023-08-11 00:00')
...
if __name__ == "__main__":
  ...
  for model_name in selected_models:
    ...
    # actuals = np.array(actuals_flattened) + baseload
